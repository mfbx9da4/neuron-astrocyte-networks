...
def randomdata():
	"""
	there are 50 samples for each cat
	16.7 is 1/3 of 50

	takes data and randomly splits into third and two thirds randomly

	"""
  	# shuffle  split positions of data randomly
	lens = [16, 17, 17]
	shuffle(lens)
	l1,l2,l3 = lens
	# shuffle data
	...
	shuffle(cat2)
	shuffle(cat3)
	# pat takes 2/3 and testpat takes 1/3 pf data
	testpat = cat1[:l1] + cat2[:l2] + cat3[:l3]
	pat = cat1[l1:] + cat2[l2:] + cat3[l3:]
	# shuffle again so different categories presented at different times
	shuffle(testpat)
	shuffle(pat)
	return pat, testpat


def randomIrisData(split):
	"""
	replaces randomdata. randomdata not deleted to avoid breaking
	programs dependent on it
	"""
	lens = [int(len(cat1) * split)] * 3
	shuffle(lens)
	l1,l2,l3 = lens
	shuffle(cat1)
	shuffle(cat2)
...
def neurgenData():
    ...
    for p in pat:
        inputs.append(p[0])
        targets.append(map(float, p[1]))
    return inputs, targets


def updateCats():
	for x in cat1:
		x.insert(2, x[1].index(1))
	for x in cat2:
		x.insert(2, x[1].index(1))
	for x in cat3:
		x.insert(2, x[1].index(1))
...
# a,b = randomdata()
# assert a is not b, 'pat is the same as testpat'
# assert len(a) == 100, 'training pattern does not have a lenghth of 100'
# assert len(b) == 50, 'test pattern does not have a length of 50'
# assert type(a[0]) is list, 'first input pattern is not a list'
# assert len(a[0]) == 3, 'first input pattern does not have a length of 3'
# assert len(a[0][0]) == 4, 'input pattern features not of len 4'
