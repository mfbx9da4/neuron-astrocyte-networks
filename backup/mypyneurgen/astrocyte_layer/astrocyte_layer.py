# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 14:50:11 2013
...
from pyneurgen.layers import Layer as neurgenLayer
from pybrain.structure.modules.neuronlayer import NeuronLayer as pybrainLayer
...
class AstrocyteLayer(object):
    def __init__(self, layer, connection, astrocyte_threshold=3,
        ...
        """
        if isinstance(layer, neurgenLayer):
            print 'is neurgen Layer'
        elif isinstance(layer, pybrainLayer):
            print 'is pybrain layer'
        else:
            print 'layer type not recognized'

        if isinstance(connection):
            print 'is neurgen layer'
        elif isinstance(connection, FullConnection):
            print 'is pybrain connection'
        else:
            print 'connection type not recognized'
    ...
    def update(self):
        """
        1. Astrocytic neuron counter (ANC) counts the decision of associated
        neuron.
        2. If ANC reaches positive or negative threshold, astrocyte is updated
        to be positively (+1) or negatively (-1) activated for a fixed
        duration of iterations, respectively.
