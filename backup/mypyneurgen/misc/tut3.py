...
if os.environ['OS'] == 'Windows_NT':
    sys.path.append('C:\\Users\\DavidAdler\\Dropbox\\' +
...
# 			  random_testing=True)
	...
	# INSERT AS A TEST!! that thresh is working
	# outs = []
	# trgs = []
	# for inpt, targt in net.get_test_data():
	# 	net.process_sample(inp, trg)
	# 	out = net.output_layer.activations()
	# 	vals = net.layers.output_layer.values()
	# 	net.output_layer.get_errors()
	# 	assert where(vals > 0.5, 1., 0.) == out, 'threshold not working'
	# 	outs.append(out)
	# 	trgs.append(trg)
