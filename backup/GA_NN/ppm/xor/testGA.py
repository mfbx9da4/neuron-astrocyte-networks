...
from GA import evolveNewPop, selectTwoIndividuals, trainPP
...
class testtrainPP(unittest.TestCase):
	def setUp(self):
		self.pop = createPop()
	...
	def testZeroAstroDur(self):
		old_ws = [[x[0].copy(), x[1].copy()] for x in self.newpopW]
		new_ws = trainPP(self.newpopW, self.pop, astro_dur=0)
