...
from astrocyte_layer import OutAstroLayer
...
def rankPop(newpopW, pop, verbose=NN.eliteN):
    errors, copy = [], []
...
def trainPP(rankedPop, pop, **kwargs):
    new_weights = [item[0] for item in rankedPop]
    for i in range(NN.pop_size):
...
def associateAstrocytes(net, **kwargs):
    hiddenAstrocyteLayer = OutAstroLayer(net.ah, net.wi, **kwargs)
    outputAstrocyteLayer = OutAstroLayer(net.ao, net.wo, **kwargs)
